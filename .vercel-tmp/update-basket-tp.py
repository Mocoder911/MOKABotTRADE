...
# Update Basket_Take_Profit to $5
s.table('tactics_settings').update({'value': {'value': 5}}).eq('key', 'Basket_Take_Profit').execute()

print('Updated Basket_Take_Profit:')
r = s.table('tactics_settings').select('key', 'value').eq('key', 'Basket_Take_Profit').execute()
if r.data:
